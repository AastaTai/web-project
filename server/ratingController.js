...
const createRating = [zValidator("json", ratingValidator), async (c) => {
...
const getRatings = async (c) => {
    return c.json(await ratingRepository.readAll());
};

const getRating = async (c) => {
    const bookId = c.req.param("bookId");
    return c.json(await ratingRepository.readOne(bookId));
};

const updateRating = [zValidator("json", ratingValidator), async (c) => {
    const bookId = c.req.param("bookId");
